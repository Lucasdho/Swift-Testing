...
- **SwiftData** (`PersistenceStack`, `@Model`, repositories, migrations) → `swift-dev`→swiftdata-pro.
- **Concurrency** around `actor ImageCaching` / `@MainActor` persistence → `swift-dev`→swift-concurrency-pro.
- **Unit tests**: Swift Testing (`#expect`) only — do not migrate to XCTest. UI tests stay XCTest.
...
- Tests run against the in-memory store (`PersistenceStack(isMemoryOnly: true)`) — never the on-disk store.
- Quote the space-containing project/scheme in every `xcodebuild` command.
